...
# integrated <- RunUMAP(integrated, reduction = "integrated_lsi", 
...
# NOTE. Resumed from here on 3/16/2024 for RNA and WNN processing
# integrated <- readRDS("/hpc/projects/data.science/yangjoon.kim/zebrahub_multiome/data/processed_data/01_Signac_processed/integrated_ATAC_rLSI_v2.RDS")

# set the Default assay of the "integrated" to be "SCT"
DefaultAssay(integrated) <- "SCT"

# Subset by dataset ID
TDR118 <- subset(integrated, subset = dataset == "TDR118")
TDR119 <- subset(integrated, subset = dataset == "TDR119")
TDR124 <- subset(integrated, subset = dataset == "TDR124")
TDR125 <- subset(integrated, subset = dataset == "TDR125")
TDR126 <- subset(integrated, subset = dataset == "TDR126")
TDR127 <- subset(integrated, subset = dataset == "TDR127")
TDR128 <- subset(integrated, subset = dataset == "TDR128")

# Step 4. Integrate the RNA objects
...
# 4-1. Load the list of objects
danio.list <- list(TDR118, TDR119, TDR124, TDR125, TDR126, TDR127, TDR128)

# 4-2. Find integration anchors and integrate data 
...
integrated[["integrated"]] <- IntegrateData(anchorset = danio.anchors, 
                                new.assay.name = "integrated",
                                dims=1:30,
                                k.weight = 30)
                                
# save the integrated object (intermediate object)
saveRDS(integrated, file = paste0("/hpc/projects/data.science/yangjoon.kim/zebrahub_multiome/data/processed_data/01_Signac_processed/","integrated_RNA_ATAC.rds")) 
print(paste("RDS object saved"))              

# 4-3. Generate an integrated embedding: run PCA on integrated (corrected) counts 
# specify that we will perform downstream analysis on the corrected data note that the
# original unmodified data still resides in the 'RNA' assay
DefaultAssay(integrated) <- "integrated"

# Run the standard workflow for visualization and clustering
seurat_combined <- ScaleData(integrated, verbose = FALSE)
seurat_combined <- RunPCA(integrated, npcs = 100, verbose = FALSE, reduction.name = "integrated_pca")
print(paste("Runnning UMAP on the integrated PCA embedding..."))

# 4-4. UMAP on integrated embbedding 
integrated <- RunUMAP(integrated, reduction = "integrated_pca", dims = 1:30,
...
integrated <- FindNeighbors(integrated, reduction = "integrated_pca", dims = 1:30)
...
saveRDS(seurat_combined, file = paste0("/hpc/projects/data.science/yangjoon.kim/zebrahub_multiome/data/processed_data/01_Signac_processed/","integrated_RNA_ATAC.rds")) 
print(paste("RDS object saved"))

# 5. Compute the weigted-nearest neighbors (WNN) embeddings for the integrated RNA and ATAC

# 5.1 Identify multimodal neighbors. These will be stored in the neighbors slot, and can be accessed using bm[['weighted.nn']]
...
SaveH5Seurat(integrated_ATAC, filename = "integrated_RNA_ATAC.h5Seurat", overwrite = TRUE)
Convert("integrated_RNA_ATAC.h5Seurat", dest = "h5ad", overwrite = TRUE)
