...
Enhanced with GPU acceleration for massive speedup on large datasets.

Examples
--------

# Same API as original - now with automatic GPU acceleration
run_maelstrom(input, "hg38", outdir)
mr = MaelstromResult(outdir)

# Explicit GPU control
run_maelstrom(input, "hg38", outdir, use_gpu=True)  # Force GPU
run_maelstrom(input, "hg38", outdir, use_gpu=False) # Force CPU
"""
...
try:
    from .gpu_moap import GPUMoap, gpu_moap
    from .gpu_rank import gpu_rankagg
    
    # Enable cuML.accel for automatic sklearn → GPU conversion
    from cuml.accel import install
    install()
    
    # Optional: Import CuPy for specific operations
    import cupy as cp
    import cupyx.scipy.stats as cp_stats
    
    GPU_AVAILABLE = True
    CUPY_AVAILABLE = True
    logger_gpu = logging.getLogger("gimme.maelstrom.gpu")
    logger_gpu.info("🚀 GPU acceleration available (cuML + CuPy)")
except ImportError as e:
    GPU_AVAILABLE = False
    CUPY_AVAILABLE = False
    logger_gpu = logging.getLogger("gimme.maelstrom")
    logger_gpu.info(f"GPU acceleration not available - using CPU: {e}")

# Original imports (always available as fallback)
...
def run_maelstrom(
    ...
    random_state=None,
    use_gpu=None,  # NEW: GPU control parameter
):
    """Find differential motifs with optional GPU acceleration.
    ...
    use_gpu : bool, optional
        Enable GPU acceleration. If None (default), GPU is used automatically 
        when available. Set to False to force CPU usage, or True to force GPU usage.
    """
    
    # GPU availability detection and method selection
    gpu_enabled = _setup_gpu_acceleration(use_gpu, methods)
    
    if gpu_enabled:
        logger.info("🚀 Starting GPU-accelerated maelstrom")
        # Use GPU methods by default if not specified
        if methods is None:
            methods = ["gpuhypergeom", "gpumwu", "gpurf"]
    else:
        logger.info("Starting maelstrom")
        # Use CPU methods by default if not specified  
        if methods is None:
            methods = Moap.list_predictors()

    # All the original preprocessing code (unchanged)
    ...
    # Non-redundant filtering (unchanged)
    if filter_redundant:
    ...
    # Method setup with GPU/CPU selection
    ...
    if cluster:
        cluster = False
        if gpu_enabled:
            # Check GPU methods for classification capability
            for method in methods:
                if method.startswith("gpu"):
                    try:
                        m = GPUMoap.create(method, ncpus=ncpus, random_state=random_state)
    ...
    # Setup experiments with GPU/CPU routing
    exps = []
    clusterfile = infile
    
    if df.shape[1] != 1:
        # More than one column - regression methods
        if gpu_enabled:
            available_regression = GPUMoap.list_regression_predictors()
        else:
            available_regression = Moap.list_regression_predictors()
            
        for method in available_regression:
            if method in methods:
                try:
                    if gpu_enabled and method.startswith("gpu"):
                        m = GPUMoap.create(method, ncpus=ncpus, random_state=random_state)
                    else:
                        m = Moap.create(method, ncpus=ncpus, random_state=random_state)
    ...
    if df.shape[1] == 1 or cluster:
        ...
        if gpu_enabled:
            available_classification = GPUMoap.list_classification_predictors()
        else:
            available_classification = Moap.list_classification_predictors()
            
        for method in available_classification:
            if method in methods:
                try:
                    if gpu_enabled and method.startswith("gpu"):
                        m = GPUMoap.create(method, ncpus=ncpus, random_state=random_state)
    ...
    # Run experiments with GPU/CPU routing
    for method, scoring, fname in exps:
        try:
            ...
            if gpu_enabled and method.startswith("gpu"):
                # Use GPU version
                _gpu_moap_with_table(
                    fname, table, outdir, method, scoring,
                    ncpus=ncpus, random_state=random_state
                )
            else:
    ...
    # Rank aggregation with GPU/CPU routing
    if len(dfs) > 1:
        if gpu_enabled:
            logger.info("🚀 GPU-accelerated rank aggregation")
        else:
            logger.info("Rank aggregation")
            
        df_p = _df_rank_aggregation(df, dfs, method=aggregation, ncpus=ncpus, use_gpu=gpu_enabled)
...
def _setup_gpu_acceleration(use_gpu, methods):
    """Setup GPU acceleration based on availability and user preference"""
    if use_gpu is False:
        return False  # User explicitly disabled GPU
    ...
    # GPU is available
    if use_gpu is True:
        return True  # User explicitly requested GPU
    
    if use_gpu is None:
        # Auto-detect: use GPU for large datasets or when GPU methods specified
        if methods and any(m.startswith("gpu") for m in methods):
            return True
        return True  # Default to GPU when available
    
    return False


def _gpu_moap_with_table(input_table, motif_table, data_dir, method, scoring, ncpus=None, random_state=None):
    """GPU version of _moap_with_table"""
...
def _df_rank_aggregation(df, dfs, method="int_stouffer", ncpus=16, use_gpu=False):
    """Enhanced rank aggregation with GPU support"""
    ...
    if use_gpu and GPU_AVAILABLE:
        # GPU-accelerated rank aggregation
        func = partial(gpu_rankagg, method=method)
    else:
        # Original CPU rank aggregation
    ...
    for name, result in zip(names, ret):
        if hasattr(result, 'iloc'):
            df_p[name] = result.iloc[:, 0]  # Handle DataFrame return
        else:
            df_p[name] = result
...
# All original visualization and result functions (unchanged)
