# An R script to join the cicero output results from individual chromosomes
# Use case: When run_02_compute_CCANs_cicero_parallelized.R script failed as the slurm job didn't detect the output files at the end.
...
# inputs:
# 1) output_path: path to save the output (peaks, and CCANs)
# 2) data_id: ID for the dataset, i.e. TDR118
# 3) peaktype: type of the peak profile. i.e. CRG_arc: Cellranger-arc peaks
# (i.e. peaks_celltype, peaks_bulk, peaks_joint)
# 4) cicero_model_dir: path where individual cicero_model files were saved.
...
if (length(args) != 4) {
  stop("Usage: Rscript run_cicero_join_chroms.R output_path data_id peaktype cicero_model_dir")
}

output_path <- args[1] 
data_id <- args[2] 
peaktype <- args[3]
cicero_model_dir <- args[4]

# Example Input arguments:
# output_path = "",
# data_id="TDR118",
# peaktype = "peaks_merged"
# cicero_model_dir = "/hpc/projects/data.science/yangjoon.kim/zebrahub_multiome/data/processed_data/cicero_slurm_outputs/data_id/"

dir.create(cicero_model_dir, showWarnings = FALSE, recursive = TRUE)
setwd(cicero_model_dir) # set as the temp working directory

# print the data_id
print(data_id)

# read the genome.df dataframe
genome.df <- readRDS(file.path(cicero_model_dir, "..", "genome_df.rds"))
...
model_files <- paste0(cicero_model_dir, "cicero_model_", chromosomes, ".rds")
...
output_file_path <- paste0(output_path, "02_", data_id, "_cicero_connections_", peaktype, "_peaks.csv")
print(paste("Output file path:", output_file_path))  # Debugging print
write.csv(x = all_connections, file = output_file_path)
