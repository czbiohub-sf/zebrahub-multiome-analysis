...
#
# Outputs:
#   outputs/marker_validation_per_celltype.csv    — per-cell-type stats
#   outputs/top20_peaks_per_celltype.csv          — all top-20 peaks with gene+marker info
#   figures/peak_parts_list/marker_recall_barplot.pdf
#   figures/peak_parts_list/gene_link_coverage_barplot.pdf
#   figures/peak_parts_list/top_examples_detail.pdf
...
if not rev_df.empty:
    rev_path = f"{OUTDIR}/marker_gene_reverse_lookup.csv"
...
sum_path = f"{OUTDIR}/marker_validation_per_celltype.csv"
...
plt.tight_layout()
for ext in ["pdf", "png"]:
    fig.savefig(f"{FIG_DIR}/gene_link_coverage_barplot.{ext}", bbox_inches="tight", dpi=150)
plt.close()
print(f"  Saved: {FIG_DIR}/gene_link_coverage_barplot.{{pdf,png}}")
...
plt.tight_layout()
for ext in ["pdf", "png"]:
    fig.savefig(f"{FIG_DIR}/marker_recall_barplot.{ext}", bbox_inches="tight", dpi=150)
plt.close()
print(f"  Saved: {FIG_DIR}/marker_recall_barplot.{{pdf,png}}")
...
plt.tight_layout()
for ext in ["pdf", "png"]:
    fig.savefig(f"{FIG_DIR}/top_examples_detail.{ext}", bbox_inches="tight", dpi=150)
plt.close()
print(f"  Saved: {FIG_DIR}/top_examples_detail.{{pdf,png}}")
