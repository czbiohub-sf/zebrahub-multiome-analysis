# %% [markdown]
# # Annotate Peak Clusters with Most Accessible Celltypes and Timepoints - Zebrafish Dataset
# 
...
# 
# **Input**: Preprocessed zebrafish peak object with UMAP computed
...
# %% Define paths
# TODO: Update these paths for your zebrafish dataset
input_path = "/path/to/zebrafish/peaks_by_pb_celltype_stage_annotated.h5ad"  # UPDATE THIS
output_dir = "/path/to/zebrafish/output/"  # UPDATE THIS
...
# %% Load the preprocessed zebrafish peak object
print("="*60)
print("Loading zebrafish peak object...")
print("="*60)
...
sc.pl.umap(peaks_by_pseudobulk, color=['leiden_coarse', 'leiden_fine'], 
           ...
           save='_zebrafish_leiden_clusters.png')
...
# The var_names should be in format: celltype_timepoint
# Adjust the parsing logic based on your zebrafish naming convention
celltype_list = []
timepoint_list = []
...
# Note: Adjust this based on your zebrafish timepoint format (hpf, dpf, etc.)
try:
    peaks_by_pseudobulk.obs['top_timepoint_numeric'] = pd.to_numeric(
        peaks_by_pseudobulk.obs['top_timepoint'], 
        errors='coerce'
    )
    print("\nTimepoint converted to numeric for visualization ✓")
    ...
    print("\nWarning: Could not convert timepoint to numeric. Using categorical instead.")
    peaks_by_pseudobulk.obs['top_timepoint_numeric'] = np.nan
...
sc.pl.umap(peaks_by_pseudobulk, color='top_celltype',
           title='Most Accessible Celltype (Zebrafish)',
           save='_zebrafish_most_access_ct.png')
...
if peaks_by_pseudobulk.obs['top_timepoint_numeric'].notna().any():
    sc.pl.umap(peaks_by_pseudobulk, color='top_timepoint_numeric',
               title='Most Accessible Timepoint (Zebrafish)',
               cmap='viridis',
               save='_zebrafish_most_access_tp.png')
else:
...
plt.tight_layout()
plt.savefig(figure_path + 'zebrafish_peak_umap_annotated_most_accessible_ct_tp.png', 
            dpi=300, bbox_inches='tight')
plt.savefig(figure_path + 'zebrafish_peak_umap_annotated_most_accessible_ct_tp.pdf', 
            bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(figure_path + 'zebrafish_peak_umap_specificity_patterns.png',
            dpi=300, bbox_inches='tight')
plt.savefig(figure_path + 'zebrafish_peak_umap_specificity_patterns.pdf',
            bbox_inches='tight')
plt.show()
...
for cluster_id in sorted(list(cluster_celltype_profiles_coarse.keys()))[:3]:
    ...
    plot_cluster_accessibility_profile(
        ...
        save_path=f"{figure_path}zebrafish_cluster_{cluster_id}_accessibility_profile.png"
    )
