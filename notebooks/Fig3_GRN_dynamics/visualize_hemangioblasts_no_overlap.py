#!/usr/bin/env python
"""
Generate overlap-free layout for hemangioblasts cluster using multiple strategies
Tests 3 approaches to eliminate node overlaps
"""
...
print("="*80)
print("OVERLAP-FREE LAYOUTS FOR HEMANGIOBLASTS (Cluster 26_11)")
print("="*80)
...
def adjust_layout_for_overlap(pos, min_distance=0.3, iterations=100):
    """
    Apply force-directed adjustment to push overlapping nodes apart
    """
    print(f"  Adjusting layout to prevent overlaps (min_distance={min_distance})...")
        ...
        for i, node1 in enumerate(nodes):
            for node2 in nodes[i+1:]:
                ...
                if distance < min_distance and distance > 0:
                    ...
                    push_direction = diff / distance
                    push_amount = (min_distance - distance) / 2

                    pos_adjusted[node1] = pos1 - push_direction * push_amount
                    pos_adjusted[node2] = pos2 + push_direction * push_amount
                    moved = True
...
def plot_subgrns_no_overlap(grn_dict, predicted_pairs, cluster_id="26_11",
                            celltype_of_interest="hemangioblasts",
                            layout_strategy="adjusted_kamada_kawai",
                            layout_params=None,
                            max_labels=50, savefig=False, filename=None,
                            max_edge_width=2.0, node_size_scale=0.5):
    """
    Plot temporal dynamics with overlap-free layout

    layout_strategy options:
    - "adjusted_kamada_kawai": Kamada-Kawai + overlap adjustment
    - "spring_high_k": Spring layout with very high k value
    - "shell": Shell layout (concentric circles)
    """
    ...
    # Compute layout based on strategy
    print(f"Computing {layout_strategy} layout...")

    if layout_strategy == "adjusted_kamada_kawai":
        scale = layout_params.get('scale', 4.0)
        min_distance = layout_params.get('min_distance', 0.35)
        adjust_iterations = layout_params.get('adjust_iterations', 100)

        # Start with Kamada-Kawai
        master_pos = nx.kamada_kawai_layout(master_G, scale=scale)
        print(f"  Initial Kamada-Kawai layout (scale={scale})")

        # Adjust for overlaps
        master_pos = adjust_layout_for_overlap(master_pos, min_distance=min_distance,
                                               iterations=adjust_iterations)

    elif layout_strategy == "spring_high_k":
        k = layout_params.get('k', 3.5)
        iterations = layout_params.get('iterations', 500)
        scale = layout_params.get('scale', 4.0)
        seed = layout_params.get('seed', 42)

        master_pos = nx.spring_layout(master_G, k=k, iterations=iterations,
                                     seed=seed, scale=scale)
        print(f"  Spring layout with k={k}, iterations={iterations}, scale={scale}")

    elif layout_strategy == "shell":
        # Create shells based on node degree (hub nodes in center)
        degrees = dict(master_G.degree())
        sorted_nodes = sorted(degrees.items(), key=lambda x: x[1], reverse=True)

        # Divide into 3 shells
        n_nodes = len(sorted_nodes)
        shell1 = [node for node, deg in sorted_nodes[:n_nodes//3]]  # High degree
        shell2 = [node for node, deg in sorted_nodes[n_nodes//3:2*n_nodes//3]]  # Medium
        shell3 = [node for node, deg in sorted_nodes[2*n_nodes//3:]]  # Low degree

        shells = [shell1, shell2, shell3]
        master_pos = nx.shell_layout(master_G, nlist=shells, scale=4.0)
        print(f"  Shell layout with {len(shells)} shells")

    else:
        raise ValueError(f"Unknown layout strategy: {layout_strategy}")
    ...
    # Title and legend
    layout_name = layout_strategy.replace('_', ' ').title()
    fig.suptitle(f'{celltype_of_interest.replace("_", " ").title()} GRN Evolution - Cluster {cluster_id}\n({len(all_nodes)} nodes, {len(all_edges)} edges) - {layout_name}',
...
# Generate layouts for hemangioblasts cluster
cluster_id = '26_11'
celltype = 'hemangioblasts'
rank = 1

print(f"\n{'='*80}")
print(f"Testing overlap-free layout strategies")
print(f"{'='*80}\n")
...
# Define layout strategies to test
strategies = [
    {
        'name': 'adjusted_kamada_kawai_v1',
        'strategy': 'adjusted_kamada_kawai',
        'params': {'scale': 4.0, 'min_distance': 0.35, 'adjust_iterations': 100},
        'description': 'Kamada-Kawai + overlap adjustment (min_dist=0.35)'
    },
    {
        'name': 'adjusted_kamada_kawai_v2',
        'strategy': 'adjusted_kamada_kawai',
        'params': {'scale': 4.5, 'min_distance': 0.40, 'adjust_iterations': 150},
        'description': 'Kamada-Kawai + aggressive overlap adjustment (min_dist=0.40)'
    },
    {
        'name': 'spring_high_k',
        'strategy': 'spring_high_k',
        'params': {'k': 3.5, 'iterations': 500, 'scale': 4.5, 'seed': 42},
        'description': 'Spring layout with very high k=3.5 for maximum repulsion'
    },
    {
        'name': 'shell',
        'strategy': 'shell',
        'params': {},
        'description': 'Shell layout (concentric circles by node degree)'
    }
]
...
for variant in strategies:
    print(f"\n{'-'*80}")
    print(f"STRATEGY: {variant['name']}")
    print(f"Description: {variant['description']}")
    print(f"{'-'*80}")

    for file_format in ['png', 'pdf']:
        try:
            filename = f"{figpath}subGRN_rank{rank}_{cluster_id}_{celltype}_{variant['name']}.{file_format}"

            subgrns, master_grn, master_pos = plot_subgrns_no_overlap(
                grn_dict=grn_dict,
                predicted_pairs=predicted_pairs,
                cluster_id=cluster_id,
                celltype_of_interest=celltype,
                layout_strategy=variant['strategy'],
                layout_params=variant['params'],
                max_labels=50,
                savefig=True,
                filename=filename,
                max_edge_width=2.0,
            ...
            print(f"  ✗ Error generating {file_format} for {variant['name']}: {e}")
            import traceback
            traceback.print_exc()
            continue

print(f"\n{'='*80}")
print("OVERLAP-FREE LAYOUT GENERATION COMPLETE!")
print(f"{'='*80}")
print(f"\nOutput directory: {figpath}")
print(f"\nGenerated {len(strategies) * 2} files:")
for variant in strategies:
    print(f"  - subGRN_rank1_26_11_hemangioblasts_{variant['name']}.png/pdf")
print("\nCompare these strategies and select the one with the best node spacing.")
