#!/usr/bin/env python
"""
Generate Kamada-Kawai layout variant with increased spacing for hemangioblasts (Cluster 26_11)
Tests different scale parameters to maximize node separation
"""
...
print("="*80)
print("KAMADA-KAWAI LAYOUT WITH INCREASED NODE SPACING")
...
def plot_subgrns_kamada_kawai(grn_dict, predicted_pairs, cluster_id="26_11",
                               ...
                               scale=2.5, max_labels=50, savefig=False, filename=None,
...
print(f"\n{'='*80}")
print(f"Generating Kamada-Kawai variants with different spacing")
print(f"{'='*80}\n")
...
scale_values = [2.5, 3.0, 3.5]

for scale_val in scale_values:
    print(f"\n{'-'*80}")
    print(f"TESTING: Kamada-Kawai with scale={scale_val}")
    print(f"{'-'*80}")

    for file_format in ['png', 'pdf']:
        try:
            filename = f"{figpath}subGRN_rank{rank}_{cluster_id}_{celltype}_kamada_kawai_scale{scale_val}.{file_format}"
            ...
            print(f"  ✗ Error generating {file_format} with scale={scale_val}: {e}")
            import traceback
            traceback.print_exc()
            continue

print(f"\n{'='*80}")
print("KAMADA-KAWAI SPACING VARIANTS COMPLETE!")
print(f"{'='*80}")
print(f"\nOutput directory: {figpath}")
print(f"\nGenerated {len(scale_values) * 2} files:")
for scale_val in scale_values:
    print(f"  - subGRN_rank1_26_11_hemangioblasts_kamada_kawai_scale{scale_val}.png/pdf")
print("\nCompare the scale variants and select the best spacing for the manuscript.")
