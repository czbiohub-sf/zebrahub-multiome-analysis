#!/usr/bin/env python
"""
Generate Kamada-Kawai layout with scale=4.0 for maximum node separation
Specifically to resolve etv2/fli1b overlap in hemangioblasts cluster 26_11
"""
...
print("="*80)
print("KAMADA-KAWAI LAYOUT WITH SCALE=4.0 (MAXIMUM SPACING)")
print("Cluster 26_11 - Hemangioblasts")
print("="*80)
...
def plot_subgrns_kamada_kawai(grn_dict, predicted_pairs, cluster_id="26_11",
                               celltype_of_interest="hemangioblasts",
                               scale=4.0, max_labels=50, savefig=False, filename=None,
                               max_edge_width=2.0, node_size_scale=0.5):
    """
    Plot temporal dynamics with Kamada-Kawai layout and custom scale
    """
    # Get all timepoints
    timepoints = []
    ...
    # Compute Kamada-Kawai layout with specified scale
    print(f"Computing Kamada-Kawai layout with scale={scale}...")
    try:
        master_pos = nx.kamada_kawai_layout(master_G, scale=scale)
        print(f"  ✓ Layout computed successfully")

        # Check distances between potentially overlapping nodes
        if 'etv2' in master_pos and 'fli1b' in master_pos:
            etv2_pos = master_pos['etv2']
            fli1b_pos = master_pos['fli1b']
            distance = np.sqrt((etv2_pos[0] - fli1b_pos[0])**2 + (etv2_pos[1] - fli1b_pos[1])**2)
            print(f"  Distance between etv2 and fli1b: {distance:.4f}")
...
# Generate layout for hemangioblasts cluster
cluster_id = '26_11'
celltype = 'hemangioblasts'
rank = 1

print(f"\n{'='*80}")
print(f"Generating Kamada-Kawai layout with scale=4.0")
print(f"{'='*80}\n")
...
for file_format in ['png', 'pdf']:
    try:
        filename = f"{figpath}subGRN_rank{rank}_{cluster_id}_{celltype}_kamada_kawai_scale4.0.{file_format}"

        subgrns, master_grn, master_pos = plot_subgrns_kamada_kawai(
            grn_dict=grn_dict,
...
print(f"{'='*80}")
print(f"\nOutput files:")
print(f"  - subGRN_rank1_26_11_hemangioblasts_kamada_kawai_scale4.0.png")
print(f"  - subGRN_rank1_26_11_hemangioblasts_kamada_kawai_scale4.0.pdf")
print("\nThis version should have maximum node separation to avoid overlaps.")
