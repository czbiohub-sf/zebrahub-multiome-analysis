#!/usr/bin/env python
"""
Generate improved layout variants for hemangioblasts temporal dynamics (Cluster 26_11)
Tests 4 different layout algorithms to fix overlapping nodes and empty center
"""
...
print("="*80)
print("IMPROVED LAYOUT VARIANTS FOR HEMANGIOBLASTS (Cluster 26_11)")
print("="*80)
...
def plot_subgrns_with_custom_layout(grn_dict, predicted_pairs, cluster_id="26_11",
                                     celltype_of_interest="hemangioblasts",
                                     layout_algorithm="spring", layout_params=None,
                                     max_labels=50, savefig=False, filename=None,
                                     max_edge_width=2.0, node_size_scale=0.5):
    """
    Plot temporal dynamics with custom layout algorithm

    Parameters:
    - layout_algorithm: "circular", "spring", "kamada_kawai", "spectral"
    - layout_params: dict with algorithm-specific parameters
    """
    ...
    # Compute layout based on algorithm choice
    print(f"Computing {layout_algorithm} layout...")

    if layout_algorithm == "circular":
        scale = layout_params.get('scale', 1.8)
        master_pos = nx.circular_layout(master_G, scale=scale)
        print(f"  Using circular layout with scale={scale}")

    elif layout_algorithm == "spring":
        k = layout_params.get('k', 1.8)
        iterations = layout_params.get('iterations', 300)
        scale = layout_params.get('scale', 1.8)
        seed = layout_params.get('seed', 42)
        master_pos = nx.spring_layout(master_G, k=k, iterations=iterations,
                                     seed=seed, scale=scale)
        print(f"  Using spring layout with k={k}, iterations={iterations}, scale={scale}")

    elif layout_algorithm == "kamada_kawai":
        scale = layout_params.get('scale', 1.8)
        try:
            master_pos = nx.kamada_kawai_layout(master_G, scale=scale)
            print(f"  Using Kamada-Kawai layout with scale={scale}")
        except Exception as e:
            print(f"  Warning: Kamada-Kawai failed ({e}), falling back to spring")
            k = layout_params.get('k', 1.8)
            master_pos = nx.spring_layout(master_G, k=k, iterations=300, seed=42, scale=scale)

    elif layout_algorithm == "spectral":
        scale = layout_params.get('scale', 1.8)
        try:
            master_pos = nx.spectral_layout(master_G, scale=scale)
            print(f"  Using spectral layout with scale={scale}")
        except Exception as e:
            print(f"  Warning: Spectral failed ({e}), falling back to spring")
            master_pos = nx.spring_layout(master_G, k=1.8, iterations=300, seed=42, scale=scale)
    ...
    # Title and legend
    layout_desc = layout_algorithm.replace('_', ' ').title()
    fig.suptitle(f'{celltype_of_interest.replace("_", " ").title()} GRN Evolution - Cluster {cluster_id}\n({len(all_nodes)} nodes, {len(all_edges)} edges) - {layout_desc} Layout',
...
print(f"\n{'='*80}")
print(f"Generating layout variants for Cluster {cluster_id} - {celltype}")
print(f"{'='*80}\n")
...
# Define layout variants to test
layout_variants = [
    {
        'name': 'circular',
        'algorithm': 'circular',
        'params': {'scale': 1.8},
        'description': 'Original circular layout'
    },
    {
        'name': 'spring_v1',
        'algorithm': 'spring',
        'params': {'k': 1.8, 'iterations': 300, 'scale': 1.8, 'seed': 42},
        'description': 'Spring layout with k=1.8 (forced for small network)'
    },
    {
        'name': 'spring_v2',
        'algorithm': 'spring',
        'params': {'k': 2.5, 'iterations': 400, 'scale': 2.0, 'seed': 42},
        'description': 'Spring layout with higher k=2.5 for more spread'
    },
    {
        'name': 'kamada_kawai',
        'algorithm': 'kamada_kawai',
        'params': {'scale': 2.0},
        'description': 'Kamada-Kawai layout (better for small networks)'
    }
]
...
for variant in layout_variants:
    ...
    for file_format in ['png', 'pdf']:
        try:
            ...
            subgrns, master_grn, master_pos = plot_subgrns_with_custom_layout(
                grn_dict=grn_dict,
                predicted_pairs=predicted_pairs,
                cluster_id=cluster_id,
                celltype_of_interest=celltype,
                layout_algorithm=variant['algorithm'],
...
print(f"\n{'='*80}")
print("LAYOUT VARIANT GENERATION COMPLETE!")
print(f"{'='*80}")
print(f"\nOutput directory: {figpath}")
print(f"\nGenerated {len(layout_variants) * 2} files:")
for variant in layout_variants:
    print(f"  - subGRN_rank1_26_11_hemangioblasts_{variant['name']}.png/pdf")
print("\nCompare the variants and select the best one for the manuscript.")
