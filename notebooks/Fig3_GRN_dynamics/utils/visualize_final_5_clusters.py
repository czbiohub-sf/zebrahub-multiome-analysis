#!/usr/bin/env python
"""
Generate NetworkX temporal evolution plots for final 5 biologically validated clusters
...
print("="*80)
print("VISUALIZING FINAL 5 BIOLOGICALLY VALIDATED CLUSTERS")
print("="*80)
...
# Load candidates
candidates_csv = figpath + "moderate_complexity_candidates.csv"
df_candidates = pd.read_csv(candidates_csv)
print(f"   ✓ Loaded {len(df_ranked)} ranked clusters")
print(f"   ✓ Loaded {len(df_candidates)} candidate clusters")
...
print("\n2. Defining visualization function...")
...
def plot_subgrns_over_time(grn_dict, predicted_pairs, cluster_id="26_8", celltype_of_interest="NMPs",
    ...
    print(f"Computing master layout for {n_master_nodes} nodes...")
    ...
    for i, (timepoint, subgrn) in enumerate(subgrns.items()):
        ...
        if len(subgrn) > 0:
            ...
            # Label nodes - use "all_tfs_plus_dynamic" strategy
...
# Final 5 biologically validated clusters
final_clusters = [
    {'cluster_id': '26_11', 'celltype': 'hemangioblasts', 'pattern': 'INCREASING',
     'name': 'Hemangioblasts - GATA-mediated fate specification', 'rank': 1},
    {'cluster_id': '28_11', 'celltype': 'enteric_neurons', 'pattern': 'INCREASING',
     'name': 'Enteric Neurons - Progressive ENS colonization', 'rank': 2},
    {'cluster_id': '17_8', 'celltype': 'lateral_plate_mesoderm', 'pattern': 'PEAK',
     'name': 'Lateral Plate Mesoderm - Transient specification', 'rank': 3},
    {'cluster_id': '33_8', 'celltype': 'differentiating_neurons', 'pattern': 'PEAK',
     'name': 'Differentiating Neurons - Neurogenesis wave', 'rank': 4},
    {'cluster_id': '29_3', 'celltype': 'PSM', 'pattern': 'DECREASING',
     'name': 'PSM - Sox factor dynamics (novel)', 'rank': 5}
]

print(f"\n3. Generating visualizations for {len(final_clusters)} final clusters...")
print("="*80)
...
for cluster_info in final_clusters:
    ...
    print(f"\n{'='*80}")
    print(f"RANK #{rank}: {cluster_info['name']}")
    print(f"Cluster {cluster_id} in {celltype} ({cluster_info['pattern']} pattern)")
    print(f"Dynamics score: {dynamics_score:.3f}")
    print(f"Developmental TFs: {', '.join(dev_tfs) if dev_tfs else 'None'}")
    print(f"{'='*80}")
    ...
    for file_format in ['png', 'pdf']:
        try:
            ...
            subgrns, master_grn, master_pos = plot_subgrns_over_time(
                ...
                debug_labels=False,
                savefig=True,
                filename=f"{figpath}subGRN_rank{rank}_{cluster_id}_{celltype}.{file_format}",
                max_edge_width=2.0,
                figsize=(15, 10),
                node_size_scale=0.5
            )
            print(f"   ✓ Saved: subGRN_rank{rank}_{cluster_id}_{celltype}.{file_format}")
...
print(f"Generated {len(final_clusters) * 2} files ({len(final_clusters)} PNG + {len(final_clusters)} PDF)")
print("\nFINAL 5 CLUSTERS FOR MANUSCRIPT:")
for cluster_info in final_clusters:
    print(f"  {cluster_info['rank']}. {cluster_info['name']}")
