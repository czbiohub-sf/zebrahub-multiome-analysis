#!/usr/bin/env python
"""
Generate NetworkX temporal evolution plots for top dynamic clusters
...
print("="*80)
print("VISUALIZING TOP DYNAMIC CLUSTERS")
print("="*80)
...
# Select top 5 clusters to visualize
top_n = 5
print(f"\n4. Generating visualizations for top {top_n} clusters...")
print("="*80)
...
for idx in range(top_n):
    ...
    print(f"\n{'='*80}")
    print(f"Rank #{idx+1}: cluster {cluster_id} in {celltype}")
    ...
    for file_format in ['png', 'pdf']:
        try:
            ...
            subgrns, master_grn, master_pos = plot_subgrns_over_time(
                ...
                debug_labels=False,
                savefig=True,
                filename=f"{figpath}subGRN_rank{idx+1}_{cluster_id}_{celltype}.{file_format}",
                max_edge_width=2.0,
                figsize=(15, 10),
                node_size_scale=0.5
            )
            print(f"   ✓ Saved: subGRN_rank{idx+1}_{cluster_id}_{celltype}.{file_format}")
...
print(f"Generated {top_n * 2} files ({top_n} PNG + {top_n} PDF)")
