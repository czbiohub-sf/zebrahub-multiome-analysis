...
#   kernelspec:
#     display_name: celloracle_shared
#     language: python
#     name: celloracle_shared
# ---
...
# %% [markdown]
# ## Using HOMER to search for TF motifs within peaks

# %%
import subprocess
from pathlib import Path

def prepare_peak_files(adata, cluster_key='manual_annotation_peaks', output_dir='motif_analysis'):
    """
    Prepare BED files for each cluster for HOMER analysis
    """
    ...
    for cluster in adata.obs[cluster_key].unique():
        ...
        bed_data = pd.DataFrame({
            'chrom': [x.split('-')[0] for x in cluster_peaks.index],
            'start': [x.split('-')[1] for x in cluster_peaks.index],
            'end': [x.split('-')[2] for x in cluster_peaks.index],
            'name': [f"peak_{i}" for i in range(len(cluster_peaks))],
            'score': [1000] * len(cluster_peaks),
            'strand': ['.'] * len(cluster_peaks)
        })
        
        # Save to file
        bed_file = os.path.join(output_dir, f'cluster_{cluster}_peaks.bed')
        bed_data.to_csv(bed_file, sep='\t', header=False, index=False)
        bed_files[cluster] = bed_file
        
    return bed_files

def run_homer_motif_analysis(bed_files, genome='danRer11', output_dir='motif_analysis', 
                           size='given', mask=True):
    """
    Run HOMER with optimized parameters for ATAC-seq peaks
    """
    results = {}
    
    for cluster, bed_file in bed_files.items():
        print(f"\nProcessing cluster: {cluster}")
        
        # Create output directory
        cluster_dir = os.path.join(output_dir, f'cluster_{cluster}_motifs')
        os.makedirs(cluster_dir, exist_ok=True)
        
        # Construct command with optimized parameters
        cmd = [
            '/hpc/mydata/yang-joon.kim/homer/bin/findMotifsGenome.pl',
            bed_file,
            genome,
            cluster_dir,
            '-size', size,
            '-len', '8,10,12',
            '-S', '25',           # Optimize top 25 motifs
            '-gc',                # Use GC normalization
            '-useNewBg',          # Use new background method
            '-p', '4',            # Use 4 processors
            '-mset', 'vertebrates' # Use vertebrate motif database
        ]
        
        if mask:
            cmd.append('-mask')
        
        print(f"Running HOMER with command:\n{' '.join(cmd)}")
        
        try:
            # Run HOMER and wait for completion
            ...
            if os.path.exists(os.path.join(cluster_dir, 'homerResults.html')):
                print(f"HOMER completed successfully for {cluster}")
                results[cluster] = cluster_dir
                
                # Print some basic results
                known_results = os.path.join(cluster_dir, 'knownResults.txt')
                if os.path.exists(known_results):
                    print(f"\nTop known motifs for {cluster}:")
                    with open(known_results) as f:
                        for i, line in enumerate(f):
                            if i == 0:  # Skip header
                                continue
                            if i > 5:    # Show top 5 motifs
                                break
                            print(line.strip())
            else:
                print(f"Warning: HOMER ran but didn't generate expected output for {cluster}")
                print("HOMER output:")
                print(process.stdout)
                
        except subprocess.CalledProcessError as e:
            print(f"Error running HOMER for cluster {cluster}")
            print(f"Exit code: {e.returncode}")
            print("Error output:")
            print(e.stderr)
    
    if not results:
        print("Warning: No successful HOMER runs!")
    else:
        print(f"\nSuccessfully processed {len(results)} clusters")
    
    return results
    
    return results
def parse_homer_results(results_dirs):
    """
    Parse HOMER motif results into a pandas DataFrame with better error handling
    """
    all_motifs = []
    
    for cluster, result_dir in results_dirs.items():
        print(f"Parsing results for cluster: {cluster}")
        
        # Read known motifs results
        known_file = os.path.join(result_dir, 'knownResults.txt')
        if os.path.exists(known_file):
            try:
                motifs = pd.read_csv(known_file, sep='\t')
                if not motifs.empty:
                    motifs['cluster'] = cluster
                    motifs['type'] = 'known'
                    all_motifs.append(motifs)
                    print(f"Found {len(motifs)} known motifs")
                else:
                    print(f"No known motifs found in {known_file}")
            except Exception as e:
                print(f"Error reading {known_file}: {str(e)}")
        else:
            print(f"Known motifs file not found: {known_file}")
        
        # Read de novo motifs results
        denovo_file = os.path.join(result_dir, 'homerResults.txt')
        if os.path.exists(denovo_file):
            try:
                motifs = pd.read_csv(denovo_file, sep='\t')
                if not motifs.empty:
                    motifs['cluster'] = cluster
                    motifs['type'] = 'de_novo'
                    all_motifs.append(motifs)
                    print(f"Found {len(motifs)} de novo motifs")
                else:
                    print(f"No de novo motifs found in {denovo_file}")
            except Exception as e:
                print(f"Error reading {denovo_file}: {str(e)}")
        else:
            print(f"De novo motifs file not found: {denovo_file}")
    
    if not all_motifs:
        print("\nNo motif results found. This could mean:")
        print("1. HOMER is still running (it can take a while)")
        print("2. HOMER encountered errors during motif finding")
        print("3. No significant motifs were found")
        print("\nCheck the cluster directories for HOMER output files:")
        for cluster, dir_path in results_dirs.items():
            print(f"{cluster}: {dir_path}")
        return pd.DataFrame()  # Return empty DataFrame instead of raising error
    
    return pd.concat(all_motifs, ignore_index=True)



# Run the complete analysis
def analyze_cluster_motifs(adata, genome='danRer11', 
                         output_dir='/hpc/projects/data.science/yangjoon.kim/zebrahub_multiome/data/processed_data/12_peak_umap_motif_analysis'):
    """
    Complete motif analysis workflow with updated output path
    """
    print("Preparing peak files...")
    bed_files = prepare_peak_files(adata, 
                                 cluster_key="manual_annotation_peaks", 
                                 output_dir=output_dir)
    
    print("Running HOMER motif analysis...")
    results_dirs = run_homer_motif_analysis(
        bed_files, 
        genome=genome, 
        output_dir=output_dir
    )
    ...
    motif_results = parse_homer_results(results_dirs)
...
# %%
# Run motif analysis
motif_results = analyze_cluster_motifs(
    peaks_pb_hvp_50k,
    genome='danRer11',
    output_dir='/hpc/projects/data.science/yangjoon.kim/zebrahub_multiome/data/processed_data/12_peak_umap_motif_analysis'
)

# Look at top motifs for each cluster
for cluster in motif_results['cluster'].unique():
    print(f"\nTop motifs for cluster {cluster}:")
    cluster_motifs = motif_results[motif_results['cluster'] == cluster]
    print(cluster_motifs.sort_values('P-value').head())
