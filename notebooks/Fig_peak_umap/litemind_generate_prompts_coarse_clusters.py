...
# %% OLD PROMPT
# ORIGINAL PROMPT STRUCTURE (before streamlining)
...
- peak-gene association is defined by first annotating the peaks with the genes whose RNA expression is highly correlated with peak accessibility across cells, then secondly with the genes whose gene body overlaps with the peak.
- The peak cluster is also annotated with the enriched transcription factor binding motifs.
- The transcription factor motif:transcription factors database is from the cisBP database, and is provided in the table below.
""")

# Data presentation with verbose introductions
...
message.append_text("Here is the table of enriched transcription factor binding motifs for the peak cluster. Each element in the table is a z-scored enrichment score for each TF motif in the peak cluster:")
message.append_table(df_clusters_motifs_cluster)

message.append_text("Here is the table of TF motifs and their associated transcription factors:")
message.append_table(df_motif_info)
