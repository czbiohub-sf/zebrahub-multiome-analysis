...
sc.pl.umap(adata_peaks, color="total_counts",
           save="_peaks_total_counts.png", palette="magma")
...
# %%
annotated_peaks["Chromosome"].values
...
adata_peaks.obs["peak_type"] = annotated_peaks["peak_type"].values
adata_peaks.obs.head()
...
props = adata_peaks.obs['peak_type'].value_counts(normalize=True) * 100
print("\nPeak type proportions:")
for peak_type, prop in props.items():
    print(f"{peak_type}: {prop:.2f}%")

# Plot UMAP colored by peak type
sc.pl.umap(adata_peaks, color='peak_type', save="_peak_type_640K_peaks.png")

# %%
peak_counts = adata_peaks.obs['peak_type'].value_counts()
peak_props = adata_peaks.obs['peak_type'].value_counts(normalize=True) * 100
